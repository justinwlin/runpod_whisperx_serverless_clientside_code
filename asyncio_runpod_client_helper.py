...
class RunpodApiClient:
    ...
    async def wait_for_api_request_completion(self, job_id, session, polling_interval=20):
    ...
    async def execute_async_api_request(self, input_payload, session, polling_interval=20):
        """ Helper method for executing an asynchronous API request. """
        job_id = await self.send_async_api_request(input_payload, session)
        return await self.wait_for_api_request_completion(job_id, session, polling_interval)

    def execute_sync_api_request(self, input_payload, polling_interval=20):
        """ Synchronously executes an API request using the provided service API. """
        async def sync_wrapper():
            async with aiohttp.ClientSession() as session:
                return await self.execute_async_api_request(input_payload, session, polling_interval)
